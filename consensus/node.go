...
func handleConsensus() {
	...
	fmt.Print("Nombre del nuevo propietario:\n")
	var Aname string
	fmt.Scanf("%s\n", &Aname)
	fmt.Print("Apellido del nuevo propietario:\n")
	var Alastname string
	fmt.Scanf("%s\n", &Alastname)
	fmt.Print("CCI del nuevo propietario:\n")
	var Acci string
	fmt.Scanf("%s\n", &Acci)
	fmt.Print("Edad del nuevo propietario:\n")
	var Aage string
	fmt.Scanf("%s\n", &Aage)
	...
	for _, remote := range remotes {
		log.Printf("%s: sending %s,%s,%s,%s to %s\n", host, Aname, Alastname, Acci, Aage, remote)
		send(remote, Frame{"vote", host, []string{Aname, Alastname, Acci, Aage}}, nil)

		file, err := os.OpenFile("sample_owners.csv", os.O_APPEND|os.O_CREATE, os.ModePerm)
		defer file.Close()
		if err != nil {
			panic(err)
		}
		...
		f, _ := file.Stat()
		if f.Size() == 0 {
			w.Write([]string{"Name", "LastName", "CCI", "Age"})
		}

		errData := w.Write([]string{Aname, Alastname, Acci, Aage})
		if errData != nil {
			log.Println(errData)
		}
